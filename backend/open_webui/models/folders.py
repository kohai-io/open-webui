...
class FolderTable:
    ...
    def get_folders_by_ids(self, folder_ids: list[str]) -> list[FolderModel]:
        """Get folders by a list of folder IDs"""
        if not folder_ids:
            return []
        with get_db() as db:
            return [
                FolderModel.model_validate(folder)
                for folder in db.query(Folder).filter(Folder.id.in_(folder_ids)).all()
            ]
