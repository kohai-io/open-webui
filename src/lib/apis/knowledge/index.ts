...
export const toggleKnowledgeSkillType = async (
	token: string,
	id: string,
	isSkill: boolean,
	skillName?: string,
	...
	const res = await fetch(`${WEBUI_API_BASE_URL}/knowledge/${id}/skill/toggle`, {
		method: 'POST',
		headers: {
			Accept: 'application/json',
			'Content-Type': 'application/json',
...
export const syncGoogleDriveFiles = async (token: string, id: string, driveToken: string) => {
	let error = null;

	const res = await fetch(`${WEBUI_API_BASE_URL}/knowledge/${id}/sync/google-drive`, {
		method: 'POST',
		headers: {
			Accept: 'application/json',
			'Content-Type': 'application/json',
