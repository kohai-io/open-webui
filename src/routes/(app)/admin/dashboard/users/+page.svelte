<script lang="ts">
	...
	import { getDashboardUsers } from '$lib/apis/dashboard';
	...
	const loadUsers = async () => {
		loading = true;
		try {
			data = await getDashboardUsers(localStorage.token, page, limit, sortBy, order);
		} catch (err) {
			console.error(err);
					...
					<tbody class="divide-y divide-gray-200 dark:divide-gray-700">
						{#each data.users as user}
							<tr
								...
								on:click={() => goto(`/admin/dashboard/users/${user.id}`)}
							>
								<td class="px-4 py-3">
								...
								<td class="px-4 py-3">
									<span class="px-2 py-1 text-xs font-medium rounded-full {user.role === 'admin' ? 'bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400' : user.role === 'pending' ? 'bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-400' : 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400'}">
										{user.role}
									</span>
								</td>
