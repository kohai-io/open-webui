...
- Welcome's shared Quick Actions component shows Search, Notes and Media above Agents on desktop and mobile. It opens the native search modal and existing Notes/Media routes; Notes follows the sidebar's feature flag and user permission checks. New description strings use the `customWelcome` translation namespace with English fallbacks.
