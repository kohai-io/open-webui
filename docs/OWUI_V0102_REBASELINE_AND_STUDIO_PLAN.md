...
| 7 | `docs/flow-extraction-contract.md`; legacy `7f562ebb5c0893a886adc02521251fce7b725cb2`; archived v0.9.4 `d62f9dc9250be3699d27941f608712d46f4c3f38`; OWUI `ecd48e2f7`; Studio `6b957d0`, `259179e`, `a18151f`, and `f31dfe1` on `codex/flows-foundation` | Legacy and unfinished-port behavior audited. The browser executor and OWUI Flow tables/routes will not be ported. The first release is a server-executed text DAG with Input, Model, Transform, and Output. Studio now owns strict definitions, immutable versions, encrypted execution history, ordered checkpoints, bounded events, execution-bound credentials, atomic claims, concurrency, heartbeats, cancellation, stale recovery, and terminal transitions. The direct completion adapter revalidates base-model access and creates no chat. The full Studio server suite has 85 passing tests; Svelte check and production build pass. Durable worker evaluation, routes, and editor remain. |
