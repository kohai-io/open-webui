...
- [x] Add version-controlled Proxmox deployment tooling. `deploy/proxmox` contains separate digest-pinned manifests, systemd units, value-free environment inventories, guarded install/update and rollback commands, health checks, and the rehearsal checklist. Proxmox execution remains open.
- [ ] Record the selected Proxmox node, LXC IDs, storage pool, internal addresses, DNS names, private image registry, reverse-proxy host, and secret-reference owners without secret values.
...
| 9 | `docs/proxmox-staging-deployment-contract.md`; `deploy/proxmox`; Proxmox Community Scripts Open WebUI LXC and install scripts as reference material only | The staging topology and deployment bundle are ready for rehearsal: separate unprivileged Debian 13 LXCs for OWUI and Studio, pinned non-root OCI images, independent local block-backed data volumes, existing reverse-proxy path routing, digest-only updates, stopped-service backups, and paired image/data rollback. Bash syntax and static safety checks pass; the manifests pass YAML formatting. No LXC has been provisioned, and Compose runtime validation remains part of the disposable-LXC rehearsal. Host, network, storage, registry, proxy, and secret-reference inputs remain operator decisions. |
| 10 | | |
