...
| 7 | `docs/flow-extraction-contract.md`; legacy `7f562ebb5c0893a886adc02521251fce7b725cb2`; archived v0.9.4 `d62f9dc9250be3699d27941f608712d46f4c3f38`; OWUI `ecd48e2f7`; Studio `6b957d0` and `259179e` on `codex/flows-foundation` | Legacy and unfinished-port behavior audited. The browser executor and OWUI Flow tables/routes will not be ported. The first release is a server-executed text DAG with Input, Model, Transform, and Output. Migration `0003_flows.sql`, strict validation, owner-scoped immutable version CRUD, conflict handling, deletion guards, and empty/existing-database migration tests pass. The direct text-completion adapter revalidates base-model access, creates no chat, dispatches once, and handles denial, malformed response, rate limit, timeout, cancellation, and failure. The full Studio server suite has 64 passing tests; Svelte check and production build pass. Worker, credential service, events, routes, and editor remain. |
