...
Current point (2026-07-11): Studio includes native Welcome, the first read-only Media browser, and an editable Svelte Flow workspace for the admitted Input, Model, Transform, and Output nodes. The user confirmed authenticated Flow execution through the streaming OWUI completion path and inspected the editable canvas. Metadata-only audit records and a representative two-user lifecycle test close the automated Phase 7 gate. The user parked timeline persistence. See `docs/HANDOFF_2026-07-11_WELCOME_TO_MEDIA.md` and `docs/flow-extraction-contract.md`.
...
- [x] Port the first constrained editor without OWUI internal stores or routes. Published Studio commit `e9accc5` adds list/create/update/delete, linear Input/Model/optional Transform/Output editing, run history, SSE progress, cancellation, and output display.
...
- [x] Add audit records without prompt, token, or secret leakage. Studio commit `bf7dd04` adds a fixed-column metadata-only audit table and transactional flow/execution lifecycle writes. Leakage tests cover names, descriptions, definitions, prompts, model IDs, idempotency keys, inputs, outputs, request IDs, and credentials.

### Fresh-start validation

- [x] Do not add a legacy Flow export or import path. Migration and representative lifecycle coverage start from empty Studio storage and use only the supported v1 definition/service surface.
- [x] Create representative supported flows from scratch in Studio fixtures and end-to-end tests. Studio commit `bf7dd04` creates two owners' flows from scratch, pins an execution to immutable version 1 while version 2 is saved, runs it through the worker, proves cross-owner hiding, cancels the second owner's run, deletes terminal data, and verifies retained redacted audit history.
...
- [x] Representative new flows can be created, versioned, executed, and deleted in Studio. The two-user lifecycle fixture and existing authenticated route/UI evidence cover the supported first-release graph.
- [x] Flow execution is user-scoped, observable, cancellable, and resumable as designed. Owner-scoped reads/events/audit, SSE tests, cancellation, deterministic checkpoint recovery, and uncertain Model-result failure all pass.
- [x] OWUI contains no Flow domain tables, editor implementation, or execution engine. Flow schemas, routes, Svelte editor, worker, and audit records exist only in Studio; the independent migration suite creates no OWUI objects.
- [x] Studio and its worker can be upgraded or rolled back independently of OWUI. The audit image was deployed by rotating only `owui-studio-media-test`, preserving `owui-studio-media-test-data` and retaining `owui-studio-media-test-pre-flow-audit` as rollback while OWUI remained untouched.
...
- [x] Submit generally useful remaining fixes upstream where practical. No remaining fix is proven against intended staging in this phase. The unprefixed MCP resolver stays out of the patch set unless staging reproduces it; only then prepare the narrow tested upstream contribution described in `docs/phase-8-minimal-owui-patch-audit.md`.
- [x] Move experiments such as Pi Gateway outside the maintained OWUI source tree. Pi Gateway is parked by user decision and remains only in the immutable legacy reference. The v0.10.2 branch has no Pi component, route, configuration, model, router, seed, package delta, or static asset.
- [x] Reduce branding and navigation changes to configuration/assets where possible. No custom branding or static asset is retained; the only navigation delta is the disabled-by-default Welcome route/sidebar seam documented in `PATCHES.md` and `docs/phase-8-minimal-owui-patch-audit.md`.
...
- [x] Every retained OWUI patch is small, single-purpose, documented, and tested. The only retained feature is the isolated, disabled-by-default Welcome UI island; its integration hook, tests, ownership, rebase, and removal procedure are recorded in `PATCHES.md` and `docs/phase-8-minimal-owui-patch-audit.md`.
- [x] Large product features no longer live in the OWUI fork. Flows and Media are Studio-owned, timeline persistence is parked for Studio, Pi Gateway remains legacy-only, and the other classified features use upstream or remain conditional contributions.
- [x] The patch series can be applied to a clean release without manual archaeology. `rebaseline/upstream-v0.10.2` is an ordered five-commit descendant of exact tag `ecd48e2f7`; the final commit is documentation-only, and `PATCHES.md` records application, verification, rebase, and rollback.
...
- [x] Select the Proxmox staging topology and deployment contract. Use separate unprivileged Debian 13 LXCs for OWUI and Studio, pinned OCI image digests, separate managed data volumes, and the existing reverse proxy. See `docs/proxmox-staging-deployment-contract.md`.
- [x] Add version-controlled Proxmox deployment tooling. `deploy/proxmox` contains separate digest-pinned manifests, systemd units, value-free environment inventories, guarded OWUI and Studio LXC provisioners, install/update and rollback commands, health checks, runbooks, and the rehearsal checklist. Both provisioners and first deployments ran successfully; Studio used corrected bundle `r3` after the initial feature-parser stop.
- [x] Record the selected Proxmox inputs without secret values. Node `pve2` runs VMID `105` (`owui-staging`) at `10.100.1.103`, with a 24 GiB root disk and separate 16 GiB managed data volume on `local-lvm`. Studio runs VMID `107` (`studio-staging`) at reserved address `10.100.1.128`, with 2 vCPU, 4 GiB RAM, a 12 GiB root disk and separate 8 GiB managed data volume on `local-lvm`. Caddy source `10.100.1.20` serves both from `owui-stage.theoldschool.house`; Keycloak realm `homelab` provides shared identity.
- [x] Build and publish the immutable non-root OWUI image with recorded digest after explicit approval. The deployed reference is `git.theoldschool.house/robert/open-webui@sha256:012e5f80e9aaf508c5831fde9440a78fea3b7ff2f4b2a8ca451758e904e696c9`, built from exact maintained commit `6dca01731` for `linux/amd64` with runtime user `1000:1000`.
- [x] Build and publish the immutable Studio image from exact commit `bf7dd04` after fresh explicit approval. The deployment reference is `git.theoldschool.house/robert/open-webui-studio@sha256:9bbb5752adb54718df8cc00aaef746aed8629d4be558f1af34e586ccf4612462`; all 130 server tests, zero-warning static checks, the production build, non-root identity, data ownership, and local `/studio/health` smoke passed before publication.
- [x] Provision empty OWUI storage in staging and complete the first digest deployment. The unprivileged Debian 13 LXC, Compose runtime, systemd unit, local health, public Caddy route, and retained Welcome page passed. See `deploy/proxmox/evidence/OWUI_REHEARSAL_2026-07-12.md`.
- [x] Provision the separate Studio LXC, database, and storage in staging. LXC `107`, Docker/Compose, the separate 8 GiB managed data volume, exact image digest, non-root identity, reviewed deployment bundle, reserved address `10.100.1.128`, root-only runtime configuration, active/enabled systemd service, local/LAN health, migration `0007_flow_audit.sql`, `studio_flow_audit`, and SQLite integrity pass. First deployment release: `/var/backups/open-webui-studio-deploy/releases/20260712T161658Z.td2Vzx`.
- [x] Complete OWUI first-admin, closed-signup, LiteLLM model/chat, and explicit reboot-persistence checks. First-admin creation, automatic signup closure, LiteLLM discovery, one authenticated chat, reboot persistence, systemd state, and local/public health pass without recording identities, prompts, responses, or secrets.
- [ ] Run core Studio, permission, performance, worker-recovery, and public `/studio/*` routing tests. Same-origin routing, public health, Keycloak Authorization Code + PKCE, OWUI provider-token exchange, hardened account linking, visible models/Agents, Media/Flows entry pages, and Flow CRUD/canvas/versioning/live progress/output/history/audit/deletion pass; isolation, cancellation, performance, SSE reconnection, and worker-recovery cases remain open.
- [ ] Rehearse rollback at the OWUI, Studio, worker, and data levels.
- [ ] Prove off-LXC backup and restore for each managed data volume. Parked by operator decision; no `vzdump`, restore, or overwrite was attempted, and fresh explicit approval remains required before any restore.
...
| 2026-07-11 | Park Pi Gateway and keep it out of Phase 8                                                                     | It is an experimental legacy interface with no required role in the maintained v0.10.2 deployment                                                                                         | A separately scoped product requirement explicitly brings it back                                                                                       |
| 2026-07-11 | Stage OWUI and Studio in separate unprivileged Debian 13 LXCs with pinned OCI images and separate data volumes | Separate LXCs preserve independent upgrade, storage, failure, and rollback boundaries while reusing the existing tested container images                                                  | The selected Proxmox host cannot support the OCI runtime without broad LXC privileges, in which case use two small VMs with the same service boundaries |
...
| 7     | `docs/flow-extraction-contract.md`; legacy `7f562ebb5c0893a886adc02521251fce7b725cb2`; archived v0.9.4 `d62f9dc9250be3699d27941f608712d46f4c3f38`; OWUI `ecd48e2f7`; published Studio commit `bf7dd04` on `codex/flows-foundation` | Phase 7 is complete. Studio owns strict definitions, immutable versions, encrypted execution history, ordered checkpoints, bounded events, execution credentials, worker claims and identity, concurrency, heartbeats, cancellation, recovery, terminal transitions, queue orchestration, the in-process runner, CRUD/execution APIs, owner-scoped SSE, fixed-column metadata-only audit history, and the editable Svelte Flow workspace. The canvas supports deterministic add/delete/connect plus in-canvas settings for Input, Model, Transform, and Output only; deferred types remain rejected. A representative two-user fixture creates, versions, executes, isolates, cancels, deletes, and audits new flows without importing legacy data. All 130 Studio server tests, zero-warning Svelte check, full ESLint, changed-file formatting, production and Docker builds, independent container rotation, migration verification, health, and logs pass. The user confirmed live execution and completed the editable-canvas confidence checklist. The planning and Studio histories through `90abaa923` and `bf7dd04` are published. |
| 8     | `docs/phase-8-minimal-owui-patch-audit.md`; `docs/mcp-oauth-google-drive-comparison.md`; `PATCHES.md` on `rebaseline/upstream-v0.10.2`                                                                                             | Phase 8 is complete for v0.10.2. The maintained OWUI branch retains only the disabled-by-default Welcome UI island on the exact upstream tag. It adds no database domain, background job, package dependency, static branding asset, Pi Gateway surface, or MCP/OAuth patch. Flows and Media are Studio-owned; timeline remains parked for Studio; Pi Gateway is parked legacy-only. The MCP unique-suffix resolver remains conditional on staging reproduction rather than entering the maintained patch set speculatively.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| 9     | `docs/HANDOFF_2026-07-12_PHASE9_PROXMOX.md`; `docs/proxmox-staging-deployment-contract.md`; `deploy/proxmox`; `deploy/proxmox/evidence/OWUI_REHEARSAL_2026-07-12.md`; `deploy/proxmox/evidence/STUDIO_REHEARSAL_2026-07-12.md` | OWUI and Studio staging are live in separate unprivileged Debian 13 LXCs `105` and `107` on `pve2`, using exact pinned `linux/amd64` Gitea digests and independent managed data volumes. OWUI bootstrap, administrator/signup controls, LiteLLM discovery/chat, reboot health, shared Keycloak identity, Studio publication/provisioning/database migration, same-origin `/studio/*` routing, authenticated catalogue/Media/Flows pages, and Flow CRUD/versioning/execution/history/audit pass. Thin-pool auto-extension/overcommit risk remains recorded. Flow cancellation, worker recovery, isolation, performance, OWUI/Studio update and paired rollback, parked off-LXC backup/restore, timings, repeat bootstrap, and final legacy routing rollback remain open. The Proxmox Community Scripts installer remains reference material only. |
| 10    |                                                                                                                                                                                                                                    |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
