...
| 6 | `docs/media-contract-review.md`; legacy `7f562ebb5c0893a886adc02521251fce7b725cb2`; OWUI `ecd48e2f7`; Studio commits `5b5bb4e`, `08a332f`, `278be05`, and `57f7673`; image `open-webui-studio:media-perf-57f7673` | Read-only comparison, typed adapter, authenticated content proxy, first Media page, and representative-library performance hardening are complete. Self-owned listing/search, administrator denial, pagination, preview/download streaming, range forwarding, MIME/header hardening, cancellation, sequential bounded scans, zero grid-body preload, navigation, UI states, 25 server tests, build, non-root container startup, and user-led authenticated preview/download checks pass. Upload, deletion, generation/processing controls, chat/folder hierarchy, prompt recovery, deleted-file race proof, two-user proof, and timeline persistence remain. No OWUI bridge is initially justified. |
| 7 | | |
