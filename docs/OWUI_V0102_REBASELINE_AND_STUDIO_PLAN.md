...
- [ ] Verify cancellation and bounded preview concurrency with a representative large library.
...
| 6 | `docs/media-contract-review.md`; legacy `7f562ebb5c0893a886adc02521251fce7b725cb2`; OWUI `ecd48e2f7`; Studio commits `5b5bb4e`, `08a332f`, and `278be05`; image `open-webui-studio:media-page-278be05` | Read-only comparison, typed adapter, authenticated content proxy, and first Media page are complete. Self-owned listing/search, administrator denial, pagination, preview/download streaming, range forwarding, MIME/header hardening, navigation, UI states, 23 server tests, build, non-root container startup, and user-led authenticated preview/download checks pass. Upload, deletion, generation/processing controls, chat/folder hierarchy, prompt recovery, two-user proof, representative-library cancellation/concurrency performance, and timeline persistence remain. No OWUI bridge is initially justified. |
| 7 | | |
