...
- [ ] Add a locked-topology Svelte Flow canvas for the admitted Input, Model, optional Transform, Output shape. Map `FlowDefinitionV1` to typed view nodes and edges, preserve node positions, edit configuration through a side panel, and keep execution state in a separate map keyed by node ID.
- [ ] Overlay SSE node progress and terminal state on the canvas without writing runtime state into saved definitions.
- [ ] After the locked canvas passes tests, permit add, delete, and connect for Input, Model, Transform, and Output nodes. Use client rules for feedback and retain the server validator as the authority.
...
| 7 | `docs/flow-extraction-contract.md`; legacy `7f562ebb5c0893a886adc02521251fce7b725cb2`; archived v0.9.4 `d62f9dc9250be3699d27941f608712d46f4c3f38`; OWUI `ecd48e2f7`; Studio through local commits `e9accc5`, `eac13a2`, and `18efba0` on `codex/flows-foundation` | Studio owns strict definitions, immutable versions, encrypted execution history, ordered checkpoints, bounded events, execution credentials, worker claims and identity, concurrency, heartbeats, cancellation, recovery, terminal transitions, queue orchestration, the in-process runner, CRUD/execution APIs, owner-scoped SSE, and the first constrained Flow workspace. The user confirmed a live authenticated run through the streaming OWUI completion path. All 117 Studio server tests, zero-warning Svelte check, lint, production build, signed-out headless route test, container health, and HTTP smoke checks pass. The next UI slice adds a typed Svelte Flow canvas with locked topology. Remote publication, audit records, and representative two-user Flow execution remain. |
