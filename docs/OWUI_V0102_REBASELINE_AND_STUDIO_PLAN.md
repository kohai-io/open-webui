...
| 5 | OWUI branch `rebaseline/upstream-v0.10.2` commit `559cd28f1`; Studio commits `0d74d7f` and `abb86cc`; `docs/welcome-feature-maintenance-options.md` | The Studio-owned agent experiment was reverted. Native OWUI Welcome is implemented as a disabled-by-default UI island using authorised executable models, accessible workspace models, active functions, and OWUI's native chat initialisation. Focused classifier tests and the production build pass. Live workspace-agent retest, favourites/order, two-user browser proof, and deployment rollback proof remain |
| 6 | | |
| 7 | | |
