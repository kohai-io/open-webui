...
Current point (2026-07-11): Studio includes native Welcome, the first read-only Media browser, and the constrained Flow workspace. The Flow UI awaits the user-led authenticated execution check. The user parked timeline persistence. See `docs/HANDOFF_2026-07-11_WELCOME_TO_MEDIA.md` and `docs/flow-extraction-contract.md`.
...
| 7 | `docs/flow-extraction-contract.md`; legacy `7f562ebb5c0893a886adc02521251fce7b725cb2`; archived v0.9.4 `d62f9dc9250be3699d27941f608712d46f4c3f38`; OWUI `ecd48e2f7`; Studio `6b957d0`, `259179e`, `a18151f`, `f31dfe1`, `53fe122`, `69029f9`, and local `e9accc5` on `codex/flows-foundation` | Studio owns strict definitions, immutable versions, encrypted execution history, ordered checkpoints, bounded events, execution credentials, worker claims and identity, concurrency, heartbeats, cancellation, recovery, terminal transitions, queue orchestration, the in-process runner, CRUD/execution APIs, owner-scoped SSE, and the first constrained Flow workspace. All 115 Studio server tests, zero-warning Svelte check, lint, production build, signed-out headless route test, container health, and HTTP smoke checks pass. Remote publication, the user-led authenticated execution check, audit records, and representative two-user Flow execution remain. |
