...
- [ ] Add the background worker runner and deployment lifecycle.
...
| 7 | `docs/flow-extraction-contract.md`; legacy `7f562ebb5c0893a886adc02521251fce7b725cb2`; archived v0.9.4 `d62f9dc9250be3699d27941f608712d46f4c3f38`; OWUI `ecd48e2f7`; Studio `6b957d0`, `259179e`, `a18151f`, `f31dfe1`, and `53fe122` on `codex/flows-foundation` | Legacy and unfinished-port behavior audited. Studio owns strict definitions, immutable versions, encrypted execution history, ordered checkpoints, bounded events, execution credentials, claims, concurrency, heartbeats, cancellation, recovery, terminal transitions, and the durable worker core. The worker evaluates Input, Model, Transform, and Output, uses the direct completion adapter once per Model node, resumes safe checkpoints, and fails uncertain Model work without redispatch. All 100 Studio server tests pass; Svelte check and production build pass. Queue orchestration, the background runner, API routes, event streaming, and UI remain. |
